...
| **5** | **⭐ 必要條件（二者同時成立）：觀察窗口內上漲 ≥ N tick，且觀察窗口買盤佔比 ≥ M%** | `tick_rise_threshold`（預設 4）、`bid_1m_pct_threshold`（預設 70%） | ✅ 數值 |
...
| 9 | 過去 60 秒**外盤量**(張) >= 5日均量 ÷ 270 × 係數 | `vol_1m_coef`（預設 1.0）；設 0 關閉。外盤量 = 成交價 ≥ 賣一的主動買單撮合量（非總成交量），確保進場時有主動買盤 | ✅ 數值 |
