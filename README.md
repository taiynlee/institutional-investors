...
│   │   │   ├── DayTradePage.tsx   # 台股當沖（8子頁：今日交易/手動買賣/交易紀錄/當沖篩選/交易設定/後台設定/當沖健診/訊息Log）
│   │   │   ├── ManualTradePage.tsx # 手動買賣（含 ManualTradeContent，嵌入 DayTradePage 手動買賣子頁）
│   │   │   ├── Result.tsx         # 篩選績效頁（日期+持有日數下拉、均漲幅、勝率 X/Y、A/B策略篩選）
│   │   │   ├── ExitAlertsPage.tsx # 退場止損（策略標籤、現價、落榜後漲跌、高水位BB、籌碼觸發）
│   │   │   ├── Holders.tsx        # 千張大戶占比排行頁（週增減%排序）
│   │   │   ├── UsStocksPage.tsx   # 美股追蹤（收盤+盤後，每 5 分鐘自動更新）
│   │   │   ├── ScoreA.tsx         # 策略A最新分頁
│   │   │   ├── ScoreB.tsx         # 策略B近3日≥60分頁
│   │   │   ├── ScoreC.tsx         # 策略C基本面加速頁
│   │   │   ├── WatchlistAPage.tsx # A追蹤清單頁（現BB、漲跌%基準切換、軟刪除狀態管理）
...
| 訊息 Log | `/fubon-api/line-notifications` | 近 5 天 LINE 通知（類型/第N次/訊息內容）；類型包含**訊號觸發（signal）**；訊息內容可點擊展開；本月已送/剩餘額度動態顯示，≤20 則顯示橙色警告 |
...
| **5** | **⭐ 外盤佔比 ≥ 門檻**：觀察窗口內外盤（主動買單成交量）佔比，確認主力在追買 | `bid_1m_pct_threshold`（預設 85%）/ `tick_window_seconds`（預設 60秒） | ✅ 數值 |
| **6** | **⭐ 外盤量 ≥ 動態基準**：觀察窗口內外盤量(張) ≥ 近5分鐘平均每分鐘總成交量 × 係數；資料不足2分鐘自動跳過 | `vol_1m_coef`（預設 0.8，設 0 關閉）/ `tick_window_seconds` | ✅ 數值 |
...
條件 2 的 check_not_in_position 可在前端「交易設定」頁面透過 checkbox 開關。條件 4、6、7 設 0 可關閉。所有數值條件皆可即時調整。
...
| **顯示欄位** | 代號、名稱、策略標籤、現價、落榜後漲跌%、現 BB、高水位 BB、籌碼3d%、最後上榜日、已消失篩選日數、訊號 badges |
| **天數顏色** | 今日剛落榜=黃色、≤2日=橘色、更早=灰色 |
| **高水位 BB** | 在榜期間曾達到的最高 BB 位階；≥8=紅色（高危），≥5=黃色，<5=灰色（相對安全） |
| **落榜後漲跌** | 以最後上榜收盤為基準，對比今日收盤的漲跌%；下跌=綠色（避掉虧損），上漲=紅色（持倉需留意） |
...
| 功能 | 說明 |
|------|------|
| **日期下拉選單** | 最多列出過去 10 個有篩選結果且已有後續收盤的交易日，標籤格式：`YYYY-MM-DD（持有N日）` |
| **持有日數** | 從篩選日到最新有收盤的實際交易日數（扣除假日），非固定次日 |
| **漲幅基準** | 以資料庫中最新有收盤的交易日收盤為準，能跨假日比對 |
| **列顏色** | AI精選+總分第一=紫色、僅AI精選=藍色、僅總分第一=黃色、其他=灰 |
| **均漲幅** | 該日篩選股當下均漲幅，依所選基準日計算 |
| **勝率** | chg_pct > 0 的股票數 / 總股票數，格式：`勝率 X/Y (N%)`；≥50%=綠色，<50%=紅色 |
| **策略篩選** | 全部 / 策略A / 策略B 三個頁籤，依 tags 欄位即時過濾（均漲幅和勝率也同步重算） |
...
| 策略 | 條件 | 說明 |
|------|------|------|
| **A** | 啟動初期（MA20斜率 0.3~2%、上軌斜率>2%、帶寬<35%、站上MA5≤5天）+ **今日首次**創 30 日新高（昨日尚未突破）+ 出量≥均量×1.5 + BB位階>5 + chip_1d≥1% OR chip_12d≥1% + 趨勢保護 | 突破當日首次訊號 |
| **B** | 近 50 個交易日內曾創 30 日新高（突破當日位階>5）+ 今日 BB 位階 ≤ 5（已回踩月線附近，與 score_b BB 評分上限一致）+ chip_6d≥1% AND chip_12d≥1% + 千張大戶本週人數≥0（未減少）+ 趨勢保護 | 創高後拉回、主力未出場 |
...
1. **啟動初期確認**：MA20 斜率 0.3%~2.0%（剛開始向上彎，非過熱）、布林上軌斜率 > 2%（上軌擴張中）、BB 帶寬 < 35%（未過度擴張）、站上 MA5 連續天數 ≤ 5（仍在上漲初期）
2. **今日首次創 30 日新高**：今日收盤突破前 30 日最高收盤，**且昨日尚未突破**（只抓突破當天，不追第二天）；收盤須在當日高低區間的上 70% 以上（過濾長上影線假突破）
3. **今日出量**：今日成交量 ≥ 20 日均量 × 1.5 倍
4. **突破有動能**：突破當日的布林位階 > 5
5. **籌碼條件**（滿足其一即可）：chip_1d ≥ 1% 且 chip_12d > 0，或 chip_1d > 0 且 chip_12d ≥ 1%
6. **趨勢保護**（必要條件，見下）

**score_a 評分（滿分 100 分）— 通過進場條件後排序用**

| 區塊 | 滿分 | 說明 |
|------|------|------|
| A. 籌碼強度 | 30 | chip_ratio_1d（今日）+ chip_ratio_12d（12日）三法人淨買超/資本額% |
| B. 突破品質 | 35 | BB位階 + 收盤在當日高低區間位置 + 漲幅 + 今日量/20日均量 |
| C. 動能品質 | 15 | 布林上軌斜率（反向：2-3%最高10分，≥8%僅2分，越陡=趨勢越延伸=進場點越差）+ MA20斜率 |
| D. 千張大戶 | 20 | TDCC週報近6週持股增減%；最新報告距今 >10 天未更新時降為中性 2 分 |

**負面懲罰（−8 分）：** `chip_ratio_1d ≤ 0` 且 `chip_ratio_12d < 0`（法人今日 + 12日雙賣超，雙確認籌碼惡化）。

> **C 區塊 upper_slope 反向說明：** 這是設計，不是 bug。策略 A 定義「啟動初期」，上軌斜率 2-3% 表示布林帶剛開始擴張，是最佳進場窗口；≥8% 代表趨勢已成熟、股價延伸，追高風險高。

#### 策略 B：創高後拉回、主力未出場

1. **歷史突破紀錄**：近 50 個交易日內，曾出現收盤突破前 30 日高點且 BB 位階 > 5 的突破事件（不含今日）
2. **目前已拉回**：今日布林位階 ≤ 5（已回踩月線附近；與 score_b BB 評分上限一致，BB > 5 在評分中得 0 分；跌破月線 BB < 0 不入場）
3. **主力未撤**：近 6 日法人淨買超 ÷ 股本 ≥ 1%，且近 12 日也 ≥ 1%
4. **大戶未跑**：千張大戶本週持股人數 ≥ 上週（w1 ≥ 0，主力確實未出場）
5. **趨勢保護**（必要條件，見下）

#### 策略 C：基本面加速（月營收 + EPS 雙加速）

掃描範圍：`stock_pool`（富邦自選股，約 80 檔）；資料新鮮度自動檢查，月營收應公告日 + 15 天後若 DB 仍未更新則拒絕輸出。

**入選條件（硬篩，全部須滿足）：**

1. **最新月營收 YoY ≥ 10%**
2. **月營收 YoY 連 2 個月加速**：`yoy_latest > yoy_prev > yoy_prev2`（3 期比較，缺一不可）
3. **近 2 季 EPS > 0**：`eps[0] > 0` 且 `eps[1] > 0`

**score_c 評分（滿分 100 分）— 通過進場條件後排序用**

| 區塊 | 滿分 | 說明 |
|------|------|------|
| 月營收 YoY 幅度 | 25 | >50%→25, >30%→20, >20%→15, >10%→8 |
| 月營收 YoY 加速幅度 | 15 | `delta=(yoy_latest−yoy_prev)+(yoy_prev−yoy_prev2)`；>20→15, >10→12, >5→9, else→6 |
| 月營收 MoM | 15 | 2月/3月固定5分（農曆效應）；其餘：>10%→15, >5%→10, >0%→5 |
| EPS QoQ（最新季） | 15 | eps[0] vs eps[1]；QoQ>20%→15, >10%→12, >5%→9, >0%→6；eps[1]<0且eps[0]>0（虧轉盈）→15；eps[1]=0且eps[0]>0→9 |
| EPS QoQ（前一季） | 15 | eps[1] vs eps[2]，同上規則 |
| EPS TTM YoY | 15 | 近4季合計 vs 前4季合計；>30%→15, >20%→12, >10%→9, >0%→5 |

> **加速幅度設計說明：** Gate 已確保方向（連2月加速），`_score_accel` 改評 **幅度** 而非方向，以此分化「剛開始加速（delta 5-10%）」vs「急速噴發（delta>20%）」兩類候選。所有通過 gate 的股票在此段至少拿 6 分，最高 15 分。

> **eps_old < 0（虧轉盈）：** 無法用百分比計算，直接給滿分 15，因為虧損轉盈利是最強轉機訊號。eps_old = 0 給 9 分（有轉正但無基期可比）。
...
| `holders_bonus` | FLOAT | 戶加：w1 — 本週 vs 上週持股%差（正=增加，負=減少） |
| `holders_w2` | FLOAT | 上週 vs 兩週前持股%差（連續週差，非對比最新週） |
| `holders_w3` | FLOAT | 兩週前 vs 三週前持股%差（連續週差，非對比最新週） |
