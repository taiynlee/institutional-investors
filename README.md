...
| **B** | 近 50 個交易日內曾創 30 日新高（突破當日位階 > 5）+ 今日 BB 位階 0~15（≥0 且 ≤15，跌破月線不入場）+ chip_6d ≥ 1% AND chip_12d ≥ 1% + 趨勢保護 | 創高後拉回訊號 |
...
1. **BB 帶寬壓縮**：最近 5 日中有 ≥ 3 日帶寬低於 20 日均帶寬的 85%（代表股價盤整蓄勢）；同時 MA20 斜率須在 0.3%~2.0% 之間（排除橫盤或斜率過陡的假突破）
...
4. **突破有動能**：突破當日的布林位階 > 5（排除弱勢假突破）
5. **籌碼條件**：突破當日外資 + 投信淨買超 ÷ 股本 ≥ 1%（當日主力積極進場），且近 12 日累積淨買超方向為正（未反手賣出）
6. **趨勢保護**（必要條件，見下）

#### 策略 B：創高後拉回（歷史訊號 + 當前位置）

1. **歷史突破紀錄**：近 50 個交易日內，曾出現收盤突破前 30 日高點且位階 > 5 的突破事件
2. **目前已拉回**：今日布林位階 0~15（≥0 且 ≤15；跌破月線 BB<0 不入場，與退場邏輯一致）
...
| **觸發條件** | 技術（BB位階<0 跌破月線）、籌碼（近3日外資+投信賣超≤-1.5%，且12日仍持續流出） |
