...
| 10 | 振幅（今日動能）>= 門檻 | `amplitude_min_pct`（預設 3%）；振幅 = (High-Low)/昨收×100% | ✅ 數值 |

條件 3、7 可在前端「交易設定」頁面透過 checkbox 開關（勾選 = 啟用；取消 = 放行不檢查）。條件 8、10 為必要條件，門檻可調。條件 9 無需額外設定，門檻隨 `entry_start_time` 自動計算。振幅代表今日股票的動能：振幅低表示盤整沒方向，不適合當沖。
