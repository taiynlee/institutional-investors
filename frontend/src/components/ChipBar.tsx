...
function ChipItem({ label, value, positive }: {
  ...
  return (
    <div className="text-center">
      <div className={`text-xs font-medium ${positive ? 'text-red-400' : 'text-green-400'}`}>{value}</div>
...
export function ChipBar({ stock }: ChipBarProps) {
  const fmtLots = (n: number) => {
    const sign = n > 0 ? '+' : ''
    return Math.abs(n) >= 1000 ? `${sign}${(n / 1000).toFixed(0)}K` : `${sign}${n.toFixed(0)}`
  }
  const fmtPct = (n: number) => `${n >= 0 ? '+' : ''}${(n * 100).toFixed(1)}%`
  return (
    <div className="grid grid-cols-5 gap-1 mt-2 p-2 bg-gray-800 rounded text-xs">
      <ChipItem label="外資6日" value={fmtLots(stock.foreign_6d_net)} positive={stock.foreign_6d_net > 0} />
