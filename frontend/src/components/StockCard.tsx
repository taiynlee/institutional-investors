...
export function StockCard({ stock: s }: StockCardProps) {
  const dot = s.score >= 80 ? 'text-green-400' : s.score >= 60 ? 'text-yellow-400' : 'text-red-400'
  const analysis = generateAnalysis(s)
  const tag = s.tags.filter(t => ['A', 'B', 'A+B'].includes(t)).join('+') || '?'
  const dipStr = s.dip_bonus > 0 ? ` +${s.dip_bonus}資` : ''
  const holderVal = Math.round(s.holders_bonus)
  const holderStr = holderVal !== 0 ? ` ${holderVal > 0 ? '+' : ''}${holderVal}戶` : ''
  const streakStr = s.streak > 1 ? `連續${s.streak}日` : `初次`
        ...
        <div className="flex-1 min-w-0">
          <div className="text-white text-sm font-bold">
            ...
            <span className="text-gray-400 font-normal">
              [{tag}] 分={s.score}{dipStr}{holderStr}
            </span>
          </div>
          <div className="text-gray-500 text-xs mt-0.5">
            {streakStr} &nbsp;BB={s.bb_position.toFixed(1)} &nbsp;chip6d={s.chip_ratio_6d.toFixed(2)}%
          </div>
        </div>
      </div>

      <PriceSparkline code={s.code} />
    </div>
  )
}
