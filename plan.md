...
def check_entry_criteria(
    ...
    B: 近50個交易日內曾創30日新高 + 今日BB位階≤5 + 趨勢保護
       （籌碼條件由 scheduler.py 外部補查：
         A: chip_ratio_1d≥1% AND chip_ratio_12d>0
         B: chip_ratio_6d≥1% AND chip_ratio_12d≥1%）
...
export function ChipBar({ stock }: ChipBarProps) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2 mt-2 p-2 bg-gray-800 rounded">
      <ChipItem label="外資6日" value={fmt(stock.foreign_6d_net)} positive={stock.foreign_6d_net > 0} />
      <ChipItem label="投信6日" value={fmt(stock.trust_6d_net)} positive={stock.trust_6d_net > 0} />
...
| 創30日新高 | Task 7 (_find_30d_high_breakout) |
...
- `check_entry_criteria` 回傳 `passes_A`, `passes_B_price`, `passes`, `bb_position`, `bb_peak`, `peak_days_ago`, `is_squeeze` = `ScreeningResult` 欄位 ✓
- A 策略籌碼：`chip_ratio_1d ≥ 1% AND chip_ratio_12d > 0`（scheduler.py 外部補查）✓
- B 策略籌碼：`chip_ratio_6d ≥ 1% AND chip_ratio_12d ≥ 1%`（scheduler.py 外部補查）✓
- tags 欄位格式：空格分隔字串（非 JSON），`r.tags.split()` 解析 ✓
